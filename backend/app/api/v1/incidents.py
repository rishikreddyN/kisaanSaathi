...
async def submit_incident_form(
    ...
    try:
        ...
        # 2. If voice audio is provided, attach audio to incident for AEO playback
        if audio_bytes and len(audio_bytes) > 0:
            try:
                # Fast path: If description is already transcribed from the frontend,
                # upload the audio directly to storage without blocking the farmer on a 25s CPU re-transcription
                if effective_description and effective_description.strip() != "Voice Agricultural Incident":
                    client = get_supabase_client()
                    if client:
                        try:
                            ...
                            logger.warning(f"[VoicePipeline] Storage upload notice: {storage_err}")
                else:
...
async def analyze_confirmed_transcript(
    ...
    try:
        ...
        return {
            ...
            "is_complete": stage1.get("is_complete", False),
            "missing_fields": stage1.get("missing_fields", []),
            "clarification_prompt": stage1.get("clarification_prompt"),
...
async def get_incident(incident_id: str):
    try:
        ...
        # Check if rejection exists in ai_records or sd
        rejection_data = sd.get("rejection")
        if rejection_data or formatted_incident.get("status") == "REJECTED":
            formatted_incident["status"] = "REJECTED"
            formatted_incident["cluster_id"] = None
            if rejection_data:
                formatted_incident["rejection_reason"] = rejection_data.get("reason")
                formatted_incident["rejected_at"] = rejection_data.get("rejected_at")
...
async def list_incidents(limit: int = 20):
    ...
    try:
        ...
        for inc in raw_list:
            ...
            ai_analyses = inc.get("ai_analysis") or []
            is_rej = fmt.get("status") == "REJECTED"
            for a in ai_analyses:
                sd = a.get("structured_data") if isinstance(a, dict) else None
                if isinstance(sd, dict) and sd.get("rejection"):
                    is_rej = True
                    fmt["rejection_reason"] = sd["rejection"].get("reason")
                    fmt["rejected_at"] = sd["rejection"].get("rejected_at")
                    break
            if is_rej:
                fmt["status"] = "REJECTED"
                fmt["cluster_id"] = None
