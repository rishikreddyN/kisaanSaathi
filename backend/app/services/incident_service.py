...
def is_incident_rejected(inc: Optional[Dict[str, Any]]) -> bool:
    """
    Determines whether an incident has been officially rejected by an AEO.
    Checks explicit status, rejection_reason attribute, or presence of rejection
    metadata across any joined ai_analysis records.
    """
    if not inc or not isinstance(inc, dict):
        return False
    if inc.get("status") == "REJECTED":
        return True
    if inc.get("rejection_reason"):
        return True
    ai_list = inc.get("ai_analysis") or []
    if isinstance(ai_list, dict):
        ai_list = [ai_list]
    if isinstance(ai_list, list):
        for a in ai_list:
            if isinstance(a, dict):
                sd = a.get("structured_data")
                if isinstance(sd, dict) and sd.get("rejection"):
                    return True
    return False
...
def get_incident_by_id(incident_id: str) -> Optional[Dict[str, Any]]:
    ...
    if response.data and len(response.data) > 0:
        inc = format_incident_location(dict(response.data[0]))
        try:
            ai_res = client.table("ai_analysis").select("structured_data").eq("incident_id", incident_id).order("created_at", desc=True).execute()
            if ai_res.data and len(ai_res.data) > 0:
                for a in ai_res.data:
                    sd = a.get("structured_data")
                    if isinstance(sd, dict) and sd.get("rejection"):
                        inc["status"] = "REJECTED"
                        inc["cluster_id"] = None
                        inc["rejection_reason"] = sd["rejection"].get("reason")
                        break
        except Exception:
            pass
        return inc
...
def update_incident_workflow_status(
    ...
    if target_status == "REJECTED":
        update_payload["cluster_id"] = None
        
    try:
        update_res = client.table("incidents").update(update_payload).eq("id", incident_id).execute()
        updated_incident = update_res.data[0] if update_res.data else dict(incident)
    except Exception as db_err:
        err_msg = str(db_err)
        if target_status == "REJECTED" and ("23514" in err_msg or "incidents_status_check" in err_msg):
            fallback_payload = {
                "status": "RESOLVED",
                "cluster_id": None,
                "resolved_at": now_iso,
            ...
            update_res = client.table("incidents").update(fallback_payload).eq("id", incident_id).execute()
            updated_incident = update_res.data[0] if update_res.data else dict(incident)
            updated_incident["status"] = "REJECTED"
            updated_incident["cluster_id"] = None
        else:
            raise db_err
...
def reject_incident(incident_id: str, reason: str, officer_id: Optional[str] = "AEO001") -> Dict[str, Any]:
    ...
    try:
        ...
        err_msg = str(db_err)
        if "23514" in err_msg or "incidents_status_check" in err_msg:
            fallback_payload = {
                "status": "RESOLVED",
                "cluster_id": None,
                "resolved_at": now_iso,
    ...
    updated_incident["status"] = "REJECTED"
    updated_incident["cluster_id"] = None
    updated_incident["rejection_reason"] = clean_reason
...
def get_map_incidents_and_clusters(
    ...
    for inc in time_filtered:
        ...
        is_rej = is_incident_rejected(inc)

        if status_filter == "all":
            if is_rej:
                continue
        elif status_filter == "new":
            if is_rej or status_val not in ("NEW", "AI_ANALYZED"):
                continue
        elif status_filter in ("acknowledged", "in_progress"):
            if is_rej or status_val not in ("ACKNOWLEDGED", "INVESTIGATING"):
                continue
        elif status_filter == "resolved":
            if is_rej or status_val not in ("RESOLVED", "ACTION_TAKEN"):
                continue
        elif status_filter == "rejected":
            if not is_rej:
                continue
        elif status_filter == "high_priority":
            if is_rej or inc.get("priority") not in ("HIGH", "CRITICAL"):
                continue
    ...
    total_count = sum(1 for i in raw_incidents if not is_incident_rejected(i))
    new_count = sum(1 for i in raw_incidents if i.get("status") in ("NEW", "AI_ANALYZED") and not is_incident_rejected(i))
    in_progress_count = sum(1 for i in raw_incidents if i.get("status") in ("ACKNOWLEDGED", "INVESTIGATING") and not is_incident_rejected(i))
    resolved_count = sum(1 for i in raw_incidents if i.get("status") in ("RESOLVED", "ACTION_TAKEN") and not is_incident_rejected(i))
    rejected_count = sum(1 for i in raw_incidents if is_incident_rejected(i))
    high_priority_count = sum(1 for i in raw_incidents if i.get("priority") in ("HIGH", "CRITICAL") and not is_incident_rejected(i))
    ...
    for inc in filtered_incidents:
        ...
        if lat is not None and lng is not None and -90 <= lat <= 90 and -180 <= lng <= 180:
            is_incident_rejected_val = is_incident_rejected(inc)
            item = {
                "id": str(inc["id"]),
                "farmer_name": farmer_name,
                "crop": inc.get("crop"),
                "description": inc.get("description", ""),
                "status": "REJECTED" if is_incident_rejected_val else inc.get("status", "NEW"),
                ...
                "cluster_id": None if is_incident_rejected_val else (str(inc.get("cluster_id")) if inc.get("cluster_id") else None),
            ...
            if not is_incident_rejected_val:
