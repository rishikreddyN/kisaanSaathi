...
class TestMultimodalQwen3VLPipeline(unittest.TestCase):
    ...
    # =========================================================================
    # TEST 16: Incomplete voice details (missing crop) sets is_complete=False
    # =========================================================================
    def test_16_incomplete_voice_details_gating(self):
        import asyncio

        async def run_test():
            # When transcript mentions symptoms ("yellowing leaves", "spots") but NO specific crop
            stage1_res = await validate_and_understand_agricultural_complaint(
                transcript="చేనులో ఆకులన్నీ పసుపు రంగులోకి మారుతున్నాయి, మచ్చలు వచ్చాయి.", # no crop name
                language_hint="Telugu"
            )

            self.assertTrue(stage1_res["agriculture_related"])
            self.assertFalse(stage1_res["is_complete"])
            self.assertIn("crop", stage1_res["missing_fields"])
            self.assertIsNotNone(stage1_res["clarification_prompt"])
            self.assertIn("పంట", stage1_res["clarification_prompt"])
            self.assertIsNone(stage1_res["photo_instructions_prompt"])

            # Now test with complete details (with crop: Tomato / టమోటా)
            stage1_complete = await validate_and_understand_agricultural_complaint(
                transcript="టమోటా చేనులో ఆకులన్నీ పసుపు రంగులోకి మారుతున్నాయి, మచ్చలు వచ్చాయి.",
                language_hint="Telugu"
            )
            self.assertTrue(stage1_complete["agriculture_related"])
            self.assertTrue(stage1_complete["is_complete"])
            self.assertEqual(stage1_complete["complaint"]["crop"], "Tomato")
            self.assertEqual(len(stage1_complete["missing_fields"]), 0)
            self.assertIsNone(stage1_complete["clarification_prompt"])
