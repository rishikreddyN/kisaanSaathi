...
export default function AeoDashboard() {
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        // AEO should not see the rejected farmer in inspection pane
        setSelectedIncident(null);
        setIncidents((prev) =>
          prev.map((i) => (i.id === updated.id ? { ...i, status: 'REJECTED', rejection_reason: finalReason, cluster_id: null } : i))
        );
        // Remove from map markers
        setMapIncidents((prev) => prev.filter((i) => i.id !== updated.id));
        // Remove from active cluster details and counts
        setClusters((prev) =>
          prev
            .map((c) => {
              if (c.incident_ids?.includes(updated.id)) {
                const nextIds = c.incident_ids.filter((id) => id !== updated.id);
                return {
                  ...c,
                  incident_ids: nextIds,
                  incident_count: Math.max(0, (c.incident_count || 1) - 1),
                };
              }
              return c;
            })
            .filter((c) => (c.incident_count || c.incident_ids?.length || 0) >= 2)
        );
  ...
  // Counts (excluding rejected complaints from active counts)
  const nonRejectedIncidents = incidents.filter(
    (i) => i.status !== 'REJECTED' && !i.rejection_reason && !i.ai_analysis?.[0]?.structured_data?.rejection
  );
  const totalCount = nonRejectedIncidents.length;
  const newCount = nonRejectedIncidents.filter((i) => i.status === 'NEW' || i.status === 'AI_ANALYZED').length;
  const inProgressCount = nonRejectedIncidents.filter(
    (i) => i.status === 'ACKNOWLEDGED' || i.status === 'INVESTIGATING' || i.status === 'IN_PROGRESS' || i.status === 'ACTION_TAKEN'
  ).length;
  const resolvedCount = nonRejectedIncidents.filter((i) => i.status === 'RESOLVED').length;
  const highPriorityCount = nonRejectedIncidents.filter((i) => i.priority === 'HIGH' || i.priority === 'CRITICAL').length;
  ...
  const filteredIncidents = incidents.filter((inc) => {
    ...
    if (statusFilter === 'rejected') {
      ...
      // Exclude rejected issues from active complaint lists
      if (isRej) return false;
